...
FG_PATH = './bird'
OBJ_NAME = 'bird'
OBJ_SIZE_RANGE = (15,120)


def random_pick(dirpath, least=1, most=6):
...
class ImgCover(object):
    """docstring for ImgCover"""

    def __init__(self, background_path, pick_list, standard_size=STANDARD_SIZE):
...
#         big_thresh = min(self.bg.shape[:2]) // 8                   # 控制覆盖上去的物体最大尺寸（小于背景图片尺寸//8）
        big_thresh = max(OBJ_SIZE_RANGE[1] / self.h_ratio, OBJ_SIZE_RANGE[1] / self.w_ratio)   # 控制最小尺寸（resize到standard size后大于15）
        small_thresh = min(OBJ_SIZE_RANGE[0]/ self.h_ratio, OBJ_SIZE_RANGE[0]/ self.w_ratio)   # 控制最小尺寸（resize到standard size后大于15）
        ...
        resize_ratio = random.uniform(min(small_thresh / smaller, big_thresh / bigger),
                                      max(small_thresh / smaller, big_thresh / bigger))
    ...
    def write(self, output_dir):
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        output_name = 'cover_' + os.path.split(self.bg_path)[-1]
...
if __name__ == '__main__':
    for bgpath in [os.path.join(BG_PATH, bg) for bg in os.listdir(BG_PATH) if
                   os.path.splitext(bg)[-1].lower() in IMGTEXT]:
        pick_list = random_pick(FG_PATH)
        imgcover = ImgCover(bgpath, pick_list)
        imgcover.write('./results')
